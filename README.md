...
- **小说**：`/novels`，写小说、按章节管理，配人物角色概念图和 AI 生成的视频（支持直接上传或粘贴 B 站/YouTube 链接），章节里可以挑选出场人物/视频、参考书目从已有书影里选，还有一键分享图，也能整本导出成 Word / PDF，章节页还能用浏览器自带的语音朗读；小说和单个章节都可以单独上锁，锁定后未登录看不了；这部分内容默认不需要登录也能看，创作、导出、朗读和被锁定的内容都需要登录
